...
----------------------------------------------------------------------------------------------------------------------------------------------------
       -------------------------------------------------------Subqueries---------------------------------------------------------
--You should not include an alias when you write a subquery in a conditional statement. 
--Because the subquery is treated as an individual value (or set of values in the IN case) rather than as a table.
...
--use DATE_TRUNC to pull month level information about the first order ever placed in the order table, 
--then find the orders that in the same month and year as the first order, and then pull the avg for each type of paper qty in this month
...
SELECT 
    ...
    spr.rep, 
    m.max_sales
FROM sales_per_rep spr
JOIN max_sales_by_region m
ON spr.region = m.region
AND spr.total_sales = m.max_sales;
...
JOIN region r
ON r.id = s.region_id
WHERE r.name = (SELECT region FROM max_region)
GROUP BY 1;
